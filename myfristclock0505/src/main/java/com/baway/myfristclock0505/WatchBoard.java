 ...
 * 3.初始化画笔
 * 4.由于表盘始终显示是圆形的,要做到图形一直在view的中间很简单,
 * 但是那样就会浪费很多的空间,于是我们应该重写onMeasure方法,
 * 使得表盘始终只占用一个正方形的空间,但是处理的前提是用户一定会给一个确定的值,
 * 不管是宽度还是高度或者两者都是.
 * 5.获取表盘圆的半径值与尾部长度值
 * 6.接下来就是最重要的绘制阶段了
 */
...
public class WatchBoard extends View{
    ...
    private void paintPointer(Canvas canvas) {
        ...
        int hour = calendar.get(java.util.Calendar.HOUR_OF_DAY); //时
        int minute = calendar.get(java.util.Calendar.MINUTE); //分
        int second = calendar.get(java.util.Calendar.SECOND); //秒
